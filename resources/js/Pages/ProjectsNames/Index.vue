...
import ProjectsNamesTable from "@/Components/ProjectsNames/ProjectsNamesTable.vue";
...
function updateStatus(status) {
  Inertia.put(route("admin.projects-names.status", status.id), status);
}
...
function deleteProject() {
  ...
  Inertia.delete(route("admin.projects-names.destroy", selectedRow.value.data.id), {
    onSuccess: () => {
      isShowingDeleteConfirmationDialog.value = false;
...
function search(event) {
  Inertia.get(route("admin.projects-names.index"), {
    search: event
  });
}

function updateInertiaRequest(params) {
    Inertia.get(route("admin.projects-names.index"), params);
}
