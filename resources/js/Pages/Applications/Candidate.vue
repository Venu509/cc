...
const applicationState = ref('pending');
const queryString = window.location.search;
const urlParams = new URLSearchParams(queryString);

applicationState.value = urlParams.get('tab') ?? 'pending';
...
function changeApplicantStatus(status) {
  ...
  Inertia.post(route("admin.applied-candidates.status", props.vacancy.id),{
    vacancy: props.vacancy.id,
    resume: props.resume.id,
    status: status,
    tab: applicationState.value,
    type: 'inertia',
