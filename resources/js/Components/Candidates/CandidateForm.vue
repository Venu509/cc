<template>
  <div class="flex flex-col gap-6">
    <FormButtonAndHeaderSection title="Add Candidate" :is-saving="isSaving" :button-text="buttonText" @save="save"/>

    <div class="card">
      <div ref="scrollContainer" class="scrollable-container overflow-x-auto whitespace-nowrap cursor-grab">
      ...
      <div v-if="!isSaving">
        ...
        <JobPreferences id="job-preferences" v-if="activeTab === 'job-preferences'" :form="form" :job-types="jobTypes"
                        :industries="industries" :employment-status="employmentStatus" :errors="errors" :user-preferred-locations="userPreferredLocations"
                        :validate-field="validateField"/>
...
const isDisabled = (tab) => {
  return intendedRoute === 'admin.candidates.create' && tab !== activeTab.value;
};
...
onMounted(() => {
  ...
  watchUrl();
  setupUrlWatcher();
  fetchRules(route("admin.candidates.rules"));
});
...
const buttonText = computed(() => {
  ...
  return route().current("admin.candidates.create") ? 'Save' : 'Next';
});
...
function save() {
  ...
  if (props.isEditing) {
    Inertia.post(route("admin.candidates.update", props.modelValue.id), form, {
      onError: handleError,
      onSuccess: handleSuccess,
    });
    return;
  }
  Inertia.post(route("admin.candidates.store"), form, {
    onError: handleError,
    onSuccess: handleSuccess,
  });
}
...
function getUpdatedCandidate(candidate) {
      return axios.get(route("admin.candidates.fetch"), {
        params: {
          candidate: candidate
        },
      })
