<template>
  <div class="flex flex-col gap-6">
    <FormButtonAndHeaderSection title="Add Branches" :is-saving="isSaving" :button-text="buttonText" @save="save"/>
      ...
      <div class="p-6">
        <div v-if="!isSaving">
          <div class="grid lg:grid-cols-2 gap-3">
            <div>
              <Label :required="true">Branch Name</Label>
              <Input
                  id="name"
                  v-model="form.name"
                  autocomplete="name"
...
function save() {
  ...
  if (props.isEditing) {
    Inertia.post(route("admin.branches.update", props.modelValue.id), form, {
      onError: handleError,
  ...
  Inertia.post(route("admin.branches.store"), form, {
    onError: handleError,
    onSuccess: handleSuccess,
  });
}
...
onMounted(() => {
  fetchRules(route("admin.branches.rules"));
});
